var CURRENT_COIN = 'SAOLA';
var PARAMS = {
	'SAOLA': {
		coingecko: 'dogecoin',
		coinjs: cc.bitcoin,
		network: cc.bitcoin.networks.saolacash,
		qrColor: '291A13',
		minFee: 0,
		maxFee: 0.1,
		txFee: 0,
	...
	'BTC': {
		...
		donation: '15JY7dXkJScBjJjJJbEX2d4cDB35SGFkJq',
	...
	'LTC': {
		...
		donation: 'LPK2i73i4pKqH6Y3yPgHz13Q8v4vXZ4d3K',
	...
	'DOGE': {
		...
		donation: 'DMcZZLGA5BHDP421WfVK6xBRL2z9hwd67b',
...
function switchCoinNow(whichCoin) {
  ...
  if(whichCoin == "SAOLA") {
     $("#address").attr("placeholder", "SAOLA address, Lottery or HODLx");
...
function spendf() {
  ...
  if(balance < FEE || SAOLA(amount+FEE) > balance) { alert("Insufficient funds! Minimum network fee is " + FEE + " " + CURRENT_COIN + "."); return; }
  ...
  } else if (CURRENT_COIN != "SAOLA") {
      alert("Lottery and/or HODL transactions only work for SAOLA!"); return;
  }
 ...
 $.ajax({
  ...
  if (address == "Lottery") {
    ...
    if (nDrawWithin >= 4960 || nDrawWithin <= 40) {
        ...
        alert("Minimum lottery ticket is 0.01 SAOLA!");
        return;
    }
  ...
  var change = SAOLA(balance - amount - FEE);
  if (change > 0) {
    // Try to avoid dusting in the SaolaCash blockchain
    if (CURRENT_COIN == "SAOLA" && change < 1 &&
        confirm("Change is less than 1 SAOLA! To avoid dusting the SaolaCash blockchain, press OK to donate it. Thank you!")) {
      donation = change;
      change = 0;
    } else
...
function createHODLRewardsTx(address, amount, result) {
  ...
  var p2sh = cc.bitcoin.address.fromOutputScript(scriptPubKey, cc.bitcoin.networks.saolacash);
